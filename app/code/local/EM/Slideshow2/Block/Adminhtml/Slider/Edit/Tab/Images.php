...
class EM_Slideshow2_Block_Adminhtml_Slider_Edit_Tab_Images extends Mage_Adminhtml_Block_Widget_Form
{
	...
	public function getTransition($choose="demo"){
	...
	public function getEasing($choose="easeOutBack"){
	...
	public function getAnimation($choose="fade"){
	...
	public function getkenb($choose=0){
		if(!isset($choose))	$choose	=	0;
		$html  =	'';
		if($choose	==	0)
	...
	public function getParallaxLevel($choose=0){
		if(!isset($choose))	$choose	=	0;
		$html  =	'';
		if($choose	==	0)
			$html .=	'<option value="0" selected>No</option>';
		...
		for($i=1;$i<=8;$i++){
			if($choose	==	$i)
				$html .=	'<option value="'.$i.'" selected>Parallax Level '.$i.'</option>';
			else
				$html .=	'<option value="'.$i.'">Parallax Level '.$i.'</option>';
		}

		return $html;
	}

	public function getVideo($choose=0){
		if(!isset($choose))	$choose	=	0;
		$html  =	'';
		if($choose	==	0)
